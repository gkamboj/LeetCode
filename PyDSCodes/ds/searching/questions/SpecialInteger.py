...
def specialInteger(arr, val):
    low, high = 1, len(arr)
    ans = 0
    while low <= high:
        mid = (high - low) // 2 + low
        if isMaxSumArrayExists(arr, mid, val):
            ans = max(ans, mid)
            low = mid + 1
        else:
            high = mid - 1
...
def isMaxSumArrayExists(arr, winSize, val):
    result = True
    currSum = 0
    for ind in range(winSize):
        currSum += arr[ind]
    if currSum > val:
        result = False
    else:
        for ind in range(winSize, len(arr)):
            currSum += arr[ind] - arr[ind - winSize]
            if currSum > val:
                result = False
                break
    return result
